/**
 * process_forward.c -- реализует обобщенную функцию обработки текста
 *
 * Copyright (c) 2017, Alexander Borodin <aborod@petrsu.ru>
 *
 * This code is licensed under a MIT-style license.
 */

#include "_text.h"
#include<string.h>
/**
 * Обрабатывает список, применяя заданную функцию к каждой строке
 * 
 * @param txt текст
 * @param process функция-обработчик
 * @returns none
 */

void process_forward_with_cursor(text txt, void (*process)(int index, char *contents))
  {   
 /* Проверяем, имеется ли текст */
 if (txt == NULL || txt->length == 0) {
     fprintf(stderr, "There are already no any lines in the text!\n");
     return;
         } 
    
  /* Текст ненулевой длины должен содержать хотя бы одну строку */
   assert(txt->begin != NULL && txt->end != NULL);
    
  /* Стартуем с начальной строки текста */
  node *current = txt->begin;
  int index = 0;
    
    /* К каждой строке текста применяем обработчик */
   while (current) {
       //если строка с курсором обрабатываем по-другому
       if(current==txt->cursor->line)
       	 {
         char buf [MAXLINE+1];
        memcpy(buf,current -> contents,txt -> cursor -> position);
        buf[txt -> cursor -> position]='|';
        memcpy(buf+txt->cursor->position+1,current->contents+txt->cursor->position,MAXLINE+1-txt->cursor->position);
        process(index,buf);
         }
       
        else
         process(index, current->contents);
         current = current->next;
	 index++;
   }
 }
void reversed_process_forward(text txt, void (*process)(int index, char *contents))
{   
    /* Проверяем, имеется ли текст */
    if (txt == NULL || txt->length == 0) {
        fprintf(stderr, "There are already no any lines in the text!\n");
        return;
    } 
    
    /* Текст ненулевой длины должен содержать хотя бы одну строку */
    assert(txt->begin != NULL && txt->end != NULL);
    
    /* Стартуем с последней строки текста */
    node *current = txt->end;
    int index = txt->length;
    
    /* К каждой строке текста применяем обработчик */
    while (current) {
        process(index, current->contents);
        current = current->previous;
        index--;
    }
}
